const baseUrl = process.env.SMOKE_BASE_URL || 'http://127.0.0.1:4830';
...
const adminCookie = await login('admin@public-safety-emergency-response.local', 'admin123');
const managerCookie = await login('manager@public-safety-emergency-response.local', 'manager123');
const analystCookie = await login('analyst@public-safety-emergency-response.local', 'analyst123');
